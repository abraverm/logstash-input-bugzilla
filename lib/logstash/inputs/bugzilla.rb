# encoding: utf-8
# rubocop:disable Style/ClassAndModuleChildren, Rails/Date
...
class LogStash::Inputs::Bugzilla < LogStash::Inputs::Base
  ...
  def run(queue)
    Stud.interval(@interval) do
      begin
        time = Time.zone.now
        add_bugs(queue, init_search('creation_time'))
        add_bugs(queue, init_search('last_change_time'))
        @last_check = time
  ...
  def add_bugs(queue, search)
    loop do
      begin
        add_event(queue, get_bug(search))
