...
timeout 120 bin/logstash/ -e 'input { generator { count => 1 } } output { elasticsearch {  elasticsearch { host => "elasticsearch" } stdout { codec => rubydebug } }'
ENDSSH
    set +x
  }
